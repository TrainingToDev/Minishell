...
int check_tokens_validity(t_token *tokens)
{
    while (tokens)
    {
        if (tokens->type == TOKEN_UNKNOWN)
        {
            ...
            return (0); // Token invalide détecté
        }
        tokens = tokens->next;
    }
    return (1);
    ...
    while (tokens)
    {
        ...
        else if (tokens->type == TOKEN_RPAREN)
        {
            ...
            if (count < 0)
            {
                ...
                return (0); // Parenthèse fermante sans correspondance
            }
        }
        tokens = tokens->next;
    }
...
{
    if (!tokens)
        return (0); // Aucun token à valider, pas de syntaxe

    if (!check_operators(tokens))
        return (0); // Erreur dans les opérateurs

    if (!check_redirections(tokens))
        return (0); // Erreur dans les redirections

    if (!check_parentheses(tokens))
        return (0); // Erreur dans les parenthèses
...
int run_shell(void)
{
    ...
    while (1)
    {
        ...
        if (*input)
        {
			// Lexer : analyse lexicale de l'entrée
            tokens = lexer(input);
			printf("out_lexer\n");
            if (!tokens)
            {
...
{
	check_args(argc, argv);      // Vérification des arguments
