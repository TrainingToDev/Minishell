...
static int output_redir(t_redir *redirs)
{
    if (redirs->type == REDIR_APPEND)
    {
        if (redir_append(redirs) == -1)
            return (-1);   
    }
    else
    {
        if (redir_out(redirs) == -1)
            return (-1);
    }
    return(0);
}
...
int redirections(t_redir *redirs, t_minishell *shell)
{
    ...
    while(current)
    {
        ...
        {
            if (output_redir(current) == -1)
                return (-1);
        }
        if (current->type == REDIR_IN)
        {
            if (redir_in(current) == -1)
                return (-1);
        }
        current = current->next;
    }
...
int redirection(t_redir *redirs, t_minishell *shell)
{
    ...
    dup2(STDOUT_FILENO, shell->fd_output);
    dup2(STDIN_FILENO, shell->fd_input);
    return(0);
}
